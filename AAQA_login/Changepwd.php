<?php
/**
 * AAQA Change Password Page
 * Allows AAQA users to change their password
 */
    ...
    try {
        ...
        if (empty($error_message)) {
            ...
            if (empty($error_message)) {
                ...
                $stmt = $conn->prepare("SELECT PASS_WORD FROM boss WHERE EMAIL = ? AND PERMISSION = 'AAQA_login' LIMIT 1");
                if (!$stmt) {
                    $error_message = 'Database error. Please try again.';
                    error_log("AAQA Changepwd.php prepare error: " . $conn->error);
                } else {
                    ...
                    if ($result && ($row = $result->fetch_assoc())) {
                        ...
                        if ($passwordMatch) {
                            ...
                            if (!$hashedPassword) {
                                ...
                                error_log("AAQA Changepwd.php password_hash failed");
                            } else {
                                // Update password using prepared statement
                                $updateStmt = $conn->prepare("UPDATE boss SET PASS_WORD = ? WHERE EMAIL = ? AND PERMISSION = 'AAQA_login'");
                                if (!$updateStmt) {
                                    $error_message = 'Database error. Please try again.';
                                    error_log("AAQA Changepwd.php update prepare error: " . $conn->error);
                                } else {
                                    ...
                                    if ($updateStmt->execute()) {
                                        ...
                                        if ($affectedRows > 0) {
                                            ...
                                            $verifyStmt = $conn->prepare("SELECT PASS_WORD FROM boss WHERE EMAIL = ? AND PERMISSION = 'AAQA_login' LIMIT 1");
                                            if ($verifyStmt) {
                                                ...
                                                if ($verifyRow = $verifyResult->fetch_assoc()) {
                                                    if (!password_verify($newPassword, $verifyRow['PASS_WORD'])) {
                                                        error_log("AAQA Changepwd.php WARNING: Password update verification failed - new password doesn't match stored hash!");
                                                    }
                                                }
                                            ...
                                            error_log("AAQA Changepwd.php update affected 0 rows for email: " . $user_email);
                                        }
                                    } else {
                                        $error_message = 'Failed to update password. Please try again.';
                                        error_log("AAQA Changepwd.php update execute error: " . $updateStmt->error);
                                    }
                                    $updateStmt->close();
                                }
                            }
        ...
        error_log("AAQA Changepwd.php Exception: " . $e->getMessage());
    } catch (Error $e) {
        $error_message = 'A fatal error occurred. Please try again.';
        error_log("AAQA Changepwd.php Fatal Error: " . $e->getMessage());
    }
}
...
<head>
    ...
    <title>Change Password - AAQA Dashboard</title>
