...
error_log("[Chairman Department] FINAL SCORES BREAKDOWN for dept_id=$dept_id:");
...
error_log("[Chairman Department] Section III: " . $auto_scores['section_3']);
error_log("[Chairman Department] Section IV: " . $auto_scores['section_4']);
error_log("[Chairman Department] Section V: " . $auto_scores['section_5']);
...
if ($expert_review) {
    if (!empty($expert_review['expert_item_scores'])) {
        ...
        error_log("[Chairman View] Loaded " . count($expert_item_scores) . " item scores");
    }
    if (!empty($expert_review['expert_narrative_scores'])) {
        ...
        error_log("[Chairman View] Loaded " . count($expert_narrative_scores) . " narrative scores");
        error_log("[Chairman View] Narrative keys: " . implode(', ', array_keys($expert_narrative_scores)));
    } else {
        error_log("[Chairman View] No expert_narrative_scores found in review");
    }
}
...
<div class="main-container">
    ...
    <div class="section-card">
        ...
        <?php endif; ?>
        <?php
        // CRITICAL: Preserve Section I value with narrative scores (calculated at line 276)
        // Store it before any includes that might overwrite it
        // Ensure we use the value that was calculated with narrative scores
        $preserved_section1_score = isset($auto_scores['section_1']) ? (float)$auto_scores['section_1'] : 0;
        error_log("[Chairman Department] Preserved Section I score before consolidated summary: " . $preserved_section1_score);
        error_log("[Chairman Department] auto_scores['section_1'] value: " . ($auto_scores['section_1'] ?? 'NOT SET'));
    ...
    try {
        ...
        error_log("[Chairman Department] Section I BEFORE including section1_faculty_output.php: " . $section1_before_include);
        ...
        // The include may have updated $section_1_auto_total, but we want to keep our narrative-included value
        $auto_scores['section_1'] = $section1_before_include;
        error_log("[Chairman Department] Section I AFTER including section1_faculty_output.php (restored): " . $auto_scores['section_1']);
        error_log("[Chairman Department] section_1_auto_total from include: " . ($section_1_auto_total ?? 'NOT SET'));
