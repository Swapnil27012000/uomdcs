                    ...
                    <?php else: ?>
                        <?php foreach ($departments as $dept): ?>
                            ...
                            $dept_name = htmlspecialchars($dept['DEPT_NAME'] ?? 'Unknown', ENT_QUOTES, 'UTF-8');
                             ...
                             // For display: "Dept Auto Score" column should show consolidated score when available
                             $base_dept_score = (float)($dept['dept_auto_score'] ?? $dept['total_score'] ?? 0);
                             $expert_score = isset($dept['expert_total_score']) ? (float)$dept['expert_total_score'] : null;
                             // Display consolidated score in "Dept Auto Score" column when available (matches consolidated score summary)
                             $dept_score = $expert_score !== null ? $expert_score : $base_dept_score;
                            ?>
                            <tr>
                                <td class="dept-name"><?php echo $dept_name; ?></td>
                                 <td>
                                     <span class="score" style="color: <?php echo $expert_score !== null ? 'var(--accent-green)' : '#6b7280'; ?>; font-weight: <?php echo $expert_score !== null ? '600' : '400'; ?>;">
                                         <?php echo number_format($dept_score, 2); ?>
                                     </span>
                                     <span style="color: #9ca3af;">/ 725</span>
                                     <?php if ($expert_score !== null): ?>
                                         <small class="d-block text-muted" style="font-size: 0.7rem;">(Consolidated)</small>
                                     <?php endif; ?>
                                 </td>
                                <td>
                                    <?php if ($expert_score !== null): ?>
