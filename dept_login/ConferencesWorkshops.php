<?php
// ConferencesWorkshops - Conferences form
...
if (isset($_POST['upload_document'])) {
    ...
    try {
        ...
        if (!$dept_id) {
            ...
            // Structure: uploads/{A_YEAR}/DEPARTMENT/{dept_id}/conferences_workshops/
            $result = handleDocumentUpload('conferences_workshops', 'Conferences and Workshops', [
                'upload_dir' => dirname(__DIR__) . "/uploads/{$A_YEAR}/DEPARTMENT/{$dept_id}/conferences_workshops/",
                'max_size' => 10,
                'document_title' => 'Conferences and Workshops Documentation',
                'srno' => $srno,
                'file_id' => $file_id
            ]);
...
if (isset($_POST['delete_document'])) {
    ...
    $A_YEAR = ($current_year - 2) . '-' . ($current_year - 1);
            ...
            $section_name = 'Conferences and Workshops';
            $get_file_query = "SELECT file_path, academic_year FROM supporting_documents 
                              WHERE dept_id = ? AND page_section = 'conferences_workshops' 
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    if ($result && mysqli_num_rows($result) > 0) {
                        ...
                        if (strpos($file_path, $project_root) === 0) {
                            ...
                            $physical_path = $project_root . "/uploads/{$doc_year}/DEPARTMENT/{$dept_id}/conferences_workshops/{$filename}";
                        }
                        ...
                        $section_name = 'Conferences and Workshops';
                        $delete_query = "UPDATE supporting_documents SET status = 'deleted', updated_date = CURRENT_TIMESTAMP WHERE academic_year = ? AND dept_id = ? AND page_section = 'conferences_workshops' AND serial_number = ? AND section_name = ? AND status = 'active'";
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        if (!isset($documents_found[$srno]) || $doc_year === $academic_year) {
                            ...
                            if (strpos($file_path, 'uploads/') === 0) {
                                ...
                                $filename = basename($file_path);
                                $file_path = '../uploads/' . $doc_year . '/DEPARTMENT/' . $dept_id . '/conferences_workshops/' . $filename;
                            }
...
if (isset($_GET['check_doc'])) {
    ...
    $A_YEAR = ($current_year - 2) . '-' . ($current_year - 1);
            ...
            $section_name = 'Conferences and Workshops';
            $check_query = "SELECT file_path, file_name, upload_date, id FROM supporting_documents WHERE academic_year = ? AND dept_id = ? AND page_section = 'conferences_workshops' AND serial_number = ? AND section_name = ? AND status = 'active' LIMIT 1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (file_exists(__DIR__ . '/csrf.php')) {
        ...
        if (function_exists('validate_csrf')) {
            ...
            if (empty($csrf) || !validate_csrf($csrf)) {
                ...
                if (!headers_sent()) {
                    header('Location: ConferencesWorkshops.php');
                }
                exit;
            }
        }
    ...
    try {
        ...
        // Validate and sanitize A section values (must be non-negative integers)
        for ($i = 1; $i <= 6; $i++) {
            $a[$i] = isset($_POST["a$i"]) ? max(0, (int)$_POST["a$i"]) : 0;
        }
        
        // Calculate total conferences (A1+A2+A3+A4+A5+A6)
        $total_conference = $a[1] + $a[2] + $a[3] + $a[4] + $a[5] + $a[6];
        
        $submitted = true;

        // Check if this is an update or insert
        if ($record_id > 0) {
            // UPDATE query using prepared statement - include TOTAL_CONFERENCE
            $query = "UPDATE `conferences_workshops` SET 
                `A1` = ?, `A2` = ?, `A3` = ?, `A4` = ?, `A5` = ?, `A6` = ?, `TOTAL_CONFERENCE` = ?
                WHERE `id` = ? AND `DEPT_ID` = ? AND `A_YEAR` = ?";
            
            $stmt = mysqli_prepare($conn, $query);
            if ($stmt) {
                // Types: 9 ints + 1 string
                mysqli_stmt_bind_param($stmt, 'iiiiiiiiis', $a[1], $a[2], $a[3], $a[4], $a[5], $a[6], $total_conference, $record_id, $dept, $A_YEAR_DB);
                if (mysqli_stmt_execute($stmt)) {
    ...
    exit;
            ...
            $check_query = "SELECT id FROM conferences_workshops WHERE DEPT_ID = ? AND A_YEAR = ? LIMIT 1";
            ...
            if ($existing_record_id > 0) {
                // UPDATE existing row - include TOTAL_CONFERENCE
                $query = "UPDATE `conferences_workshops` SET 
                    `A1` = ?, `A2` = ?, `A3` = ?, `A4` = ?, `A5` = ?, `A6` = ?, `TOTAL_CONFERENCE` = ?
                    WHERE `id` = ? AND `DEPT_ID` = ? AND `A_YEAR` = ?";
                ...
                if ($stmt) {
                    // Types: 9 ints + 1 string
                    mysqli_stmt_bind_param($stmt, 'iiiiiiiiis', $a[1], $a[2], $a[3], $a[4], $a[5], $a[6], $total_conference, $existing_record_id, $dept, $A_YEAR_DB);
                    if (mysqli_stmt_execute($stmt)) {
                        $affected_rows = mysqli_stmt_affected_rows($stmt);
                        mysqli_stmt_close($stmt);
                ...
                $check_duplicate_query = "SELECT id FROM conferences_workshops WHERE DEPT_ID = ? AND A_YEAR = ? LIMIT 1";
                ...
                // INSERT new row - include TOTAL_CONFERENCE
                $query = "INSERT INTO `conferences_workshops` 
                    (`A_YEAR`, `DEPT_ID`, `A1`, `A2`, `A3`, `A4`, `A5`, `A6`, `TOTAL_CONFERENCE`)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                
                $stmt = mysqli_prepare($conn, $query);
                if ($stmt) {
                    mysqli_stmt_bind_param($stmt, 'siiiiiiii', $A_YEAR_DB, $dept, $a[1], $a[2], $a[3], $a[4], $a[5], $a[6], $total_conference);
                    if (mysqli_stmt_execute($stmt)) {
...
if ($action === 'clear') {
    ...
    $get_files_query = "SELECT file_path FROM supporting_documents WHERE academic_year = ? AND dept_id = ? AND page_section = 'conferences_workshops' AND status = 'active'";
    ...
    $delete_docs_query = "UPDATE supporting_documents SET status = 'deleted', updated_date = CURRENT_TIMESTAMP WHERE academic_year = ? AND dept_id = ? AND page_section = 'conferences_workshops' AND status = 'active'";
    ...
    $clear_query = "DELETE FROM conferences_workshops WHERE DEPT_ID = ? AND A_YEAR = ?";
...
if ($action === 'delete' && $edit_id) {
    ...
    $delete_query = "DELETE FROM conferences_workshops WHERE id = ? AND DEPT_ID = ?";
...
if ($action === 'edit' && $edit_id) {
    $edit_query = "SELECT * FROM conferences_workshops WHERE id = ? AND DEPT_ID = ?";
...
$existing_data_query = "SELECT * FROM conferences_workshops WHERE DEPT_ID = ? AND A_YEAR = ? LIMIT 1";
...
if ($existing_result && mysqli_num_rows($existing_result) > 0) {
    ...
    $form_locked = true; // Lock form if ANY data exists (A or B columns)
    // Populate form with existing A section data
    if (!$editRow) {
        $editRow = $existing_row; // Use existing_row to populate form
    }
    $a[1] = (int)($existing_row['A1'] ?? 0);
    $a[2] = (int)($existing_row['A2'] ?? 0);
    $a[3] = (int)($existing_row['A3'] ?? 0);
    $a[4] = (int)($existing_row['A4'] ?? 0);
    $a[5] = (int)($existing_row['A5'] ?? 0);
    $a[6] = (int)($existing_row['A6'] ?? 0);
    $submitted = true;
...
<div class="container-fluid">
    <div class="glass-card">
        <form class="modern-form" method="post" id="mainForm" onsubmit="return Validate()" onkeydown="return handleKeyDown(event)">
                <div class="page-header">
                    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
                        <div>
                            <h1 class="page-title">
                                <i class="fas fa-calendar-alt me-3"></i>Conferences and Workshops
                            </h1>
                        </div>
                        <a href="export_page_pdf.php?page=ConferencesWorkshops" target="_blank" class="btn btn-warning" style="margin-left: 20px; white-space: nowrap;">
        ...
        displaySkipFormButton('conferences_workshops', 'Conferences and Workshops', $A_YEAR, $has_existing_data);
        ?>
        ...
        <!-- Conferences, Workshops, STTP and Seminars -->
        <div class="card mb-4 p-3">
            ...
            <tr>
                <td>1</td>
                            <td>Number of Industry–Academia Innovative practices/Workshop conducted during the last year</td>
                            <td><input type="number" class="form-control text-center" name="a1" min="0" value="<?= ($editRow || (isset($existing_row) && $form_locked)) ? (int)(($editRow ?? $existing_row ?? [])['A1'] ?? 0) : '0' ?>" onkeyup="calculateTotalA()" <?php echo $form_locked ? 'disabled' : ''; ?>></td>
            </tr>
            <tr>
                <td>2</td>
                            <td>Number of Workshops/STTP/Refresher or Orientation Programme Organized</td>
                            <td><input type="number" class="form-control text-center" name="a2" min="0" value="<?= ($editRow || (isset($existing_row) && $form_locked)) ? (int)(($editRow ?? $existing_row ?? [])['A2'] ?? 0) : '0' ?>" onkeyup="calculateTotalA()" <?php echo $form_locked ? 'disabled' : ''; ?>></td>
            </tr>
            <tr>
                <td>3</td>
                            <td>Number of National Conferences/Seminars/Workshops organized</td>
                            <td><input type="number" class="form-control text-center" name="a3" min="0" value="<?= ($editRow || (isset($existing_row) && $form_locked)) ? (int)(($editRow ?? $existing_row ?? [])['A3'] ?? 0) : '0' ?>" onkeyup="calculateTotalA()" <?php echo $form_locked ? 'disabled' : ''; ?>></td>
            </tr>
            <tr>
                <td>4</td>
                            <td>Number of International Conferences/Seminars/Workshops organized</td>
                            <td><input type="number" class="form-control text-center" name="a4" min="0" value="<?= ($editRow || (isset($existing_row) && $form_locked)) ? (int)(($editRow ?? $existing_row ?? [])['A4'] ?? 0) : '0' ?>" onkeyup="calculateTotalA()" <?php echo $form_locked ? 'disabled' : ''; ?>></td>
            </tr>
            <tr>
                <td>5</td>
                            <td>Number of Teachers invited as speakers/resource persons/Session Chair</td>
                            <td><input type="number" class="form-control text-center" name="a5" min="0" value="<?= ($editRow || (isset($existing_row) && $form_locked)) ? (int)(($editRow ?? $existing_row ?? [])['A5'] ?? 0) : '0' ?>" onkeyup="calculateTotalA()" <?php echo $form_locked ? 'disabled' : ''; ?>></td>
            </tr>
            <tr>
                <td>6</td>
                            <td>Number of Teachers who presented at Conferences/Seminars/Workshops</td>
                            <td><input type="number" class="form-control text-center" name="a6" min="0" value="<?= ($editRow || (isset($existing_row) && $form_locked)) ? (int)(($editRow ?? $existing_row ?? [])['A6'] ?? 0) : '0' ?>" onkeyup="calculateTotalA()" <?php echo $form_locked ? 'disabled' : ''; ?>></td>
                        </tr>
                        <tr class="table-info">
                            <td colspan="2" class="text-end fw-bold">Total</td>
                            <td><input type="text" class="form-control text-center fw-bold" id="totalA" readonly value="<?= ($editRow || (isset($existing_row) && $form_locked)) ? (int)(($editRow ?? $existing_row ?? [])['TOTAL_CONFERENCE'] ?? 0) : '0' ?>"></td>
            </tr>
                    </tbody>
        </table>
            </div>
        ...
        <div class="form-group mt-4 pdf-upload-section">
            <label class="form-label">Supporting Documents for Industry-Academia Workshops <span class="text-danger">*</span></label>
            <div class="row">
                <div class="col-md-6">
                    <label class="form-label">Industry-Academia Innovative practices/Workshop Documentation</label>
                    <div class="input-group">
                        <input type="file" class="form-control" id="industry_academia_pdf" name="industry_academia_pdf" accept=".pdf" 
                               <?php echo $form_locked ? 'disabled' : ''; ?> onchange="validatePDF(this)">
                        <button type="button" class="btn btn-outline-primary" onclick="uploadDocument('industry_academia_pdf', 1)" 
                                <?php echo $form_locked ? 'disabled' : ''; ?>>Upload</button>
                    </div>
                    <div id="industry_academia_pdf_status" class="mt-2"></div>
                </div>
            </div>
        </div>

        <div class="form-group mt-4 pdf-upload-section">
            <label class="form-label">Supporting Documents for STTP/Refresher Programs <span class="text-danger">*</span></label>
            <div class="row">
                <div class="col-md-6">
                    <label class="form-label">Workshops/STTP/Refresher Programme Documentation</label>
                    <div class="input-group">
                        <input type="file" class="form-control" id="sttp_refresher_pdf" name="sttp_refresher_pdf" accept=".pdf" 
                               <?php echo $form_locked ? 'disabled' : ''; ?> onchange="validatePDF(this)">
                        <button type="button" class="btn btn-outline-primary" onclick="uploadDocument('sttp_refresher_pdf', 2)" 
                                <?php echo $form_locked ? 'disabled' : ''; ?>>Upload</button>
                    </div>
                    <div id="sttp_refresher_pdf_status" class="mt-2"></div>
                </div>
            </div>
        </div>

        <div class="form-group mt-4 pdf-upload-section">
            <label class="form-label">Supporting Documents for National Conferences <span class="text-danger">*</span></label>
            <div class="row">
                <div class="col-md-6">
                    <label class="form-label">National Conferences/Seminars/Workshops Documentation</label>
                    <div class="input-group">
                        <input type="file" class="form-control" id="national_conferences_pdf" name="national_conferences_pdf" accept=".pdf" 
                               <?php echo $form_locked ? 'disabled' : ''; ?> onchange="validatePDF(this)">
                        <button type="button" class="btn btn-outline-primary" onclick="uploadDocument('national_conferences_pdf', 3)" 
                                <?php echo $form_locked ? 'disabled' : ''; ?>>Upload</button>
                    </div>
                    <div id="national_conferences_pdf_status" class="mt-2"></div>
                </div>
            </div>
        </div>

        <div class="form-group mt-4 pdf-upload-section">
            <label class="form-label">Supporting Documents for International Conferences <span class="text-danger">*</span></label>
            <div class="row">
                <div class="col-md-6">
                    <label class="form-label">International Conferences/Seminars/Workshops Documentation</label>
                    <div class="input-group">
                        <input type="file" class="form-control" id="international_conferences_pdf" name="international_conferences_pdf" accept=".pdf" 
                               <?php echo $form_locked ? 'disabled' : ''; ?> onchange="validatePDF(this)">
                        <button type="button" class="btn btn-outline-primary" onclick="uploadDocument('international_conferences_pdf', 4)" 
                                <?php echo $form_locked ? 'disabled' : ''; ?>>Upload</button>
                    </div>
                    <div id="international_conferences_pdf_status" class="mt-2"></div>
                </div>
            </div>
        </div>

        <div class="form-group mt-4 pdf-upload-section">
            <label class="form-label">Supporting Documents for Teachers as Speakers <span class="text-danger">*</span></label>
            <div class="row">
                <div class="col-md-6">
                    <label class="form-label">Teachers as Speakers/Resource Persons Documentation</label>
                    <div class="input-group">
                        <input type="file" class="form-control" id="teachers_speakers_pdf" name="teachers_speakers_pdf" accept=".pdf" 
                               <?php echo $form_locked ? 'disabled' : ''; ?> onchange="validatePDF(this)">
                        <button type="button" class="btn btn-outline-primary" onclick="uploadDocument('teachers_speakers_pdf', 5)" 
                                <?php echo $form_locked ? 'disabled' : ''; ?>>Upload</button>
                    </div>
                    <div id="teachers_speakers_pdf_status" class="mt-2"></div>
                </div>
            </div>
        </div>

        <div class="form-group mt-4 pdf-upload-section">
            <label class="form-label">Supporting Documents for Teachers Presentations <span class="text-danger">*</span></label>
            <div class="row">
                <div class="col-md-6">
                    <label class="form-label">Teachers Conference/Seminar Presentations Documentation</label>
                    <div class="input-group">
                        <input type="file" class="form-control" id="teachers_presentations_pdf" name="teachers_presentations_pdf" accept=".pdf" 
                               <?php echo $form_locked ? 'disabled' : ''; ?> onchange="validatePDF(this)">
                        <button type="button" class="btn btn-outline-primary" onclick="uploadDocument('teachers_presentations_pdf', 6)" 
                                <?php echo $form_locked ? 'disabled' : ''; ?>>Upload</button>
                    </div>
                    <div id="teachers_presentations_pdf_status" class="mt-2"></div>
                </div>
            </div>
        </div>
...
function calculateTotalA() {
    let total = 0;
    for (let i = 1; i <= 6; i++) {
        const value = parseInt(document.querySelector(`input[name="a${i}"]`).value) || 0;
        total += value;
    }
...
function uploadDocument(fileId, srno) {
    ...
    .catch(error => {
        ...
        console.error('[Conferences] Fetch error for upload:', error);
        return { 
            ok: false, 
    ...
    .then(response => {
        ...
        if (!response || typeof response.text !== 'function') {
            console.error('[Conferences] Invalid response object:', response);
        ...
        return response.text().then(text => {
            ...
            console.error('[Conferences] Error reading response text:', textError);
    ...
    .then(data => {
        ...
        if (data.success) {
            // Check if form is locked dynamically by checking input field's disabled state
            const formInput = document.querySelector('input[name="a1"]');
...
function checkDocumentStatus(fileId, srno) {
    ...
    .then(data => {
        ...
        if (data.success && data.file_path) {
            ...
            var deleteButton = isFormLockedNow ? '' : `<button type="button" class="btn btn-sm btn-outline-danger ms-2" onclick="deleteDocument('${escapedFileId4}', ${srno})">
                        <i class="fas fa-trash"></i> Delete
                    </button>`;
            ...
            statusDiv.innerHTML = `
                <div class="d-flex align-items-center">
                    ...
                    ${deleteButton}
                    <a href="${escapedViewPath4}" target="_blank" class="btn btn-sm btn-outline-primary ms-1">
                        <i class="fas fa-eye"></i> View
                    </a>
                </div>
            `;
...
// CRITICAL PERFORMANCE FIX: Load ALL documents in ONE batch request instead of 6 individual requests
...
function loadExistingDocuments() {
    ...
    // CRITICAL: Use consolidated endpoint with rate limiting - ONE request instead of 6
    executeWithRateLimit(() => {
    ...
    .then(data => {
        if (data.success && data.documents) {
            ...
            const documentMap = {
                'industry_academia_pdf': 1,
                'sttp_refresher_pdf': 2,
                'national_conferences_pdf': 3,
                'international_conferences_pdf': 4,
                'teachers_speakers_pdf': 5,
                'teachers_presentations_pdf': 6
            };
...
function confirmClearData() {
    if (confirm('Are you sure you want to clear all data? This action cannot be undone.')) {
        window.location.href = 'ConferencesWorkshops.php?action=clear';
    }
}
</script>
<?php
