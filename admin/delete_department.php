...
try {
    ...
    if ($dept_id > 0) {
        try {
            ...
            error_log("Admin DeleteDepartment: Error fetching department info - " . $e->getMessage());
        ...
        try {
            ...
            foreach ($check_tables as $table => $query) {
                try {
                    ...
                    if (!$stmt) {
                        error_log("Admin DeleteDepartment: Failed to prepare query for table: $table");
                    ...
                    if (!$stmt->execute()) {
                        error_log("Admin DeleteDepartment: Failed to execute query for table: $table");
                        $stmt->close();
                        continue; // Skip this table, continue with others
                    }
                    ...
                    error_log("Admin DeleteDepartment: Error checking dependencies for table $table - " . $e->getMessage());
            ...
            error_log("Admin DeleteDepartment: Error in dependency check - " . $e->getMessage());
            // Don't block the page, just log the error
        }
    }
} catch (Exception $e) {
    error_log("Admin DeleteDepartment: Fatal error - " . $e->getMessage());
    $error_message = 'An error occurred. Please try again.';
} catch (Error $e) {
    error_log("Admin DeleteDepartment: Fatal PHP error - " . $e->getMessage());
...
if ($confirm_delete && $dept_id > 0 && $dept_info && isset($conn) && $conn) {
    ...
    if (empty($error_message) && $dept_id > 0) {
        try {
            ...
            error_log("Admin DeleteDepartment: Error verifying department - " . $e->getMessage());
    ...
    if (empty($error_message) && $dept_id > 0 && $dept_info) {
        ...
        if (!$conn->begin_transaction()) {
            ...
            error_log("Admin DeleteDepartment: Failed to begin transaction");
        } else {
            try {
                $dept_code = $dept_info['DEPT_COLL_NO'] ?? 0;
                $dept_id_str = (string)$dept_id;
                ...
                foreach ($delete_queries as $table => $query) {
                    try {
                        $stmt = $conn->prepare($query);
                        if (!$stmt) {
                            error_log("Admin DeleteDepartment: Failed to prepare delete query for table: $table - " . $conn->error);
                        ...
                        if (!$stmt->execute()) {
                            error_log("Admin DeleteDepartment: Failed to execute delete for table: $table - " . $stmt->error);
                            $stmt->close();
                        ...
                        error_log("Admin DeleteDepartment: Error deleting from table $table - " . $e->getMessage());
                ...
                error_log("Admin DeleteDepartment Error: " . $e->getMessage());
...
<head>
    ...
    <title>Delete Department - Admin</title>
        ...
        <div id="page-content-wrapper">
            <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <h2 class="fs-2 m-0">Delete Department</h2>
                </div>
                                            ...
                                            if (isset($conn) && $conn) {
                                                ...
                                                if ($dept_stmt) {
                                                    if ($dept_stmt->execute()) {
                                                        ...
                                                        error_log("Admin DeleteDepartment: Failed to execute department query - " . $dept_stmt->error);
                                                    }
                                                    $dept_stmt->close();
                                                } else {
                                                    error_log("Admin DeleteDepartment: Failed to prepare department query - " . $conn->error);
                                                }
                                            } else {
                                                error_log("Admin DeleteDepartment: Database connection not available");
                                            }
                                        } catch (Exception $e) {
                                            error_log("Admin DeleteDepartment: Error loading departments - " . $e->getMessage());
                                            echo '<option value="">Error loading departments. Please refresh the page.</option>';
                                        } catch (Error $e) {
                                            error_log("Admin DeleteDepartment: Fatal error loading departments - " . $e->getMessage());
