...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_btn'])) {
    ...
    if (!empty($deptCollNo) && !empty($deptName) && !empty($deptEmail)) {
        ...
        // Sanitize inputs
        $deptCollNo = trim($deptCollNo);
        $deptName = trim($deptName);
        $deptEmail = trim($deptEmail);
        $hodEmail = trim($hodEmail ?? '');
        $hodName = trim($hodName ?? '');
        $hodAddress = trim($hodAddress ?? '');
        
        $checkQuery = "SELECT DEPT_ID FROM department_master WHERE DEPT_COLL_NO = ?";
        $checkStmt = $conn->prepare($checkQuery);
        if (!$checkStmt) {
            echo json_encode(["status" => "error", "msg" => "❌ Database prepare error."]);
            exit;
        }
...
if (!empty($department_collno)) {
    ...
    if (!$stmt) {
        echo '<p style="color:red;">Error preparing query.</p>';
        exit;
    }
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($collegeStmt) {
            ...
            $collegeInfo = $collegeResult ? $collegeResult->fetch_assoc() : null;
...
// Note: Do not close $conn here as it may be used by other scripts
?>
